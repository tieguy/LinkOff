...
// Adds "Select all for deletion" functionality

import { SELECTORS } from '../filters/selectors.js'
import { waitForClassName } from '../utils/dom.js'

/**
 * Select all messages for deletion
 * Modified from https://gist.github.com/twhitacre/d4536183c22a2f5a8c7c427df04acc90
 */
const selectMessagesForDeletion = async () => {
  const container = document.querySelector(SELECTORS.MESSAGES_LIST)
...
/**
 * Add message selection button to dropdown menu
 */
export const setupDeleteMessagesButton = async () => {
  ...
  const containerClass = SELECTORS.MESSAGES_DROPDOWN.replace(/^\./, '')
  await waitForClassName(containerClass)
  const menuContainer = document.querySelector(SELECTORS.MESSAGES_DROPDOWN_INNER)
  ...
  const observer = new MutationObserver(function () {
    ...
    selectMenuItem.onclick = function () {
      document.querySelector(SELECTORS.MESSAGES_TITLE_BUTTON).click()

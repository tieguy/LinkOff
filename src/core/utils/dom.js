// DOM manipulation utilities for LinkOff
// Shared across all filter modules
...
/**
 * Check if element contains skeleton placeholders (still loading)
 */
...
/**
 * Wait for selector to load (no longer skeleton)
 */
...
/**
 * Force callback execution with retry logic
 */
...
/**
 * Hide/dim elements by class name
 */
...
/**
 * Show elements by class name (remove hide/dim)
 */
...
/**
 * Build combined selectors for querying posts by state
 */
...
export const resetJobs = () => {
  ...
  let posts = document.querySelectorAll(getCustomSelectors(JOB_SELECTORS, 'all'))
...
/**
 * Hide/show ancestor element based on child class
 */
export const hideAncestorByChildClassName = async (
  childClassName,
  ancestorSelector,
  mode,
  showIcon = true
...
export const showAncestorByChildClassName = async (
  childClassName,
  ancestorSelector
) => {
  const elements = await waitForClassName(childClassName)
